...
describe 'webpack_asset_paths' do
  ...
  describe '#webpack_chunk_path' do
    before do
      allow(Webpack::Rails::Manifest).to receive(:chunk_path).with('chunk').and_return('/public/chunk.js')
    end

    context 'when dev_server is enabled' do
      before do
        ::Rails.configuration.webpack.dev_server.enabled = true
        ::Rails.configuration.webpack.dev_server.host = 'webpack.host'
        ::Rails.configuration.webpack.dev_server.port = 4000
      ...
      it 'returns the named chunk' do
        expect(webpack_chunk_path('chunk')).to eq('http://webpack.host:4000/public/chunk.js')
      end
    end

    context 'when dev_server is disabled' do
      before do
        ::Rails.configuration.webpack.dev_server.enabled = false
      end

      it 'returns the named chunk' do
        expect(webpack_chunk_path('chunk')).to eq('/public/chunk.js')
      end
    end
  end
end
